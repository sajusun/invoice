...
<div class="max-w-3xl mx-auto bg-white rounded shadow p-6">
    ...
    @endif
    <form id="customerForm" class="space-y-4" @if(Route::currentRouteName() === 'customers.update')method="post" action="{{route('customers.update',$customer->id)}}" @endif>
        @csrf
        <div>
        ...
        @if(Route::currentRouteName() === 'customers.update')
            <div class="flex space-x-4">
                <button type="submit" onclick="saveCustomer()"
                        class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Update
                </button>
                <button type="button" onclick="goBack()"
                        class="bg-gray-300 text-gray-800 px-4 py-2 rounded hover:bg-gray-400">Back to List
                </button>
            </div>
        @endif
