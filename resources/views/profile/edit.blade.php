...
<x-app-layout>
    ...
    <div class="max-w-lg my-4 mx-auto space-y-4" x-data="{open:false}">
            ...
            <form method="post" action="{{ route('profile.update') }}"
                ...
                <div class="profile-pic-container flex justify-center"
                    ...
                    @if(Auth::user()->social_login)
                        <img id="profilePic" src="{{ Auth::user()->profile_pic}}" alt="Profile Picture">
                    @else
                        <img id="profilePic" src="{{ Auth::user()->profile_pic ? asset('storage/profile_pics/' .
                     Auth::user()->profile_pic) : asset('storage/' . 'profile_pics/profile.png')}}"
                             alt="Profile Picture">
                    @endif

                    <input type="file" name="profile_pic" class="hidden" id="profilePicInput" accept="image/*"
        ...
        <div class="shadow bg-white p-4 rounded-md">
            ...
            <div class="pt-4 flex justify-end">
                <button class="px-6 py-2 my-4 ml-4 bg-red-400 text-white rounded-md" @click="open=!open">Delete
                    Account
                </button>
            </div>
        </div>
        <div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50" style="display: none"
             id="popup" x-show="open">
            <div class="p-4 max-w-lg bg-white rounded-md shadow-md">
                <form method="post" action="{{ route('profile.destroy') }}" class="p-6">
                    ...
                    <div class="mt-6 w-full">
                        ...
                        <input id="password" name="password" type="password" class="w-full rounded-md"
                               placeholder="{{ __('Password') }}"/>
                    </div>

                    <div class="py-4">
                        <button class="px-4 py-2 text-white bg-red-400 rounded-md" onclick="confirmDelete()">Confirm
                        </button>
                        <button class="px-4 py-2 mx-2 text-white bg-gray-600 rounded-md" onclick="closePopup()">Cancel
                        </button>
                    </div>
                </form>
            </div>
