<div class="max-w-3xl mx-auto bg-white p-4 shadow-lg rounded-md mt-1 print:shadow-none print:p-0" id="printable">
    ...
    <div class="mt-1">
        ...
        <div class="flex justify-end mt-2 text-sm text-gray-500">
            <div class="w-full px-1 text-sm text-gray-400">
            ...
            <div class="w-full max-w-xs">
                @if ($invoice_data['need_tax'])
                    <div class="flex justify-between py-1">
                        <span>Subtotal :</span>
                        <span>{{ $invoice_data['total_amount'] / (1 + $invoice_data['tax_amount'] / 100) }}
                            {{ $invoice_data->currency }}</span>
                    </div>
                    <div class="flex justify-between py-1">
                        <span>Tax ({{ $invoice_data['tax_amount'] }}%) :</span>
                        <span>{{ ($invoice_data['total_amount'] / (1 + $invoice_data['tax_amount'] / 100) / 100) * $invoice_data['tax_amount'] }}
                            {{ $invoice_data->currency }}</span>
                    </div>
                @endif
                ...
                @if ($invoice_data['status'] != 'Paid')
                    <div class="flex justify-between py-0.5 font-normal text-red-400 border-t border-gray-50 mt-0">
                        <span>Due :</span>
                        <span>{{ $invoice_data['total_amount'] - $invoice_data['paid_amount'] }}
                            {{ $invoice_data->currency }}</span>
                    </div>
                @endif
